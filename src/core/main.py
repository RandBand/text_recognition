...
def detect_result(det_file, rec_file, ocr_keys_file, image_path):
    '''
    :param det_file: 字体检测模型
    :param rec_file: 字体识别模型
    :param ocr_keys_file: #字体映射文件, 将预测的值映射字体
    :param image_path: 图片路径
    :return: 识别结果：
    {image_path:[{"box":box1, "txt":txt1}, {"box"box2, "txt":txt2}, ...], }
    '''
    ...
    for box, txt in zip(boxes, txts):
        ...
        ymin = np.min(box[:, 1])
        ymax = np.max(box[:, 1])
        box_left_right = [xmin, ymin, xmax, ymax]
        results[image_path].append({"box":box_left_right, "txt":txt})
    return results
...
if __name__=='__main__':
    ...
    parser.add_argument('-d', '--det_path', type=str, default="det.onnx", help='text detection model path')
    parser.add_argument('-r', '--rec_path', type=str, default="rec.onnx", help='text recognition model path')
    parser.add_argument('-o', '--ocr_keys_file', type=str, default="ppocr_keys_v1.txt", help='word map file')
    parser.add_argument('-i', '--image_path', type=str, default="imgs/1.jpg", required=True, help='image path')
    args = parser.parse_args()
    results = detect_result(args.det_path, args.rec_path, args.ocr_keys_file, args.image_path)
    print(results)
